...
import 'package:firebase_auth/firebase_auth.dart'; // Added to capture active buyer credentials safely
...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  // Dynamic extraction variables for current operational contexts
  List<CartItemModel> _cartItems = [];
  double _cartTotalAmount = 0.0;
  bool _isLoading = true;

  // Target active logged in user UID to query items securely
  final String _buyerUid = FirebaseAuth.instance.currentUser?.uid ?? "guest_buyer";
  ...
  /// Load pricing information dynamically using your specific database services
    ...
    switch (_currentStep) {
      ...
      case 2:
        return ReviewStep(
          ...
          savedAddress: _savedAddress,
          cartItems: _cartItems,       // Pass down items array parameter
          totalAmount: _cartTotalAmount, // Pass down calculations parameter
        );
  ...
  /// Process submission events safely through transaction batches
  Future<void> _onPrimaryAction() async {
    if (_currentStep == 0) {
      ...
      try {
        ...
        // Convert List<CartItemModel> to List<Map<String, dynamic>> to support your service parameter type
        List<Map<String, dynamic>> formattedItemsArray = _cartItems.map((item) => item.toJson()).toList();

        // Save layout execution parameters locally before cart clears out entirely
        final totalSnapshot = _cartTotalAmount;
        final shippingDestination = _savedAddress;

        // Mocking a sequential prefix tracking handle to display in our local client success routing safely
        final summaryDisplayOrderId = "ORD-${DateTime.now().millisecondsSinceEpoch.toString().substring(7)}";

        // Invoke your exact asynchronous OrderService implementation logic layout
        await _orderService.placeOrder(
          buyerId: _buyerUid,
          buyerName: _savedName,
          buyerPhone: _savedPhone,
          buyerAddress: shippingDestination,
          cartItems: formattedItemsArray,
        );
          ...
          Navigator.pushReplacement(
            ...
            MaterialPageRoute(
              builder: (_) => OrderConfirmationScreen(
                orderId: summaryDisplayOrderId,
                orderTotal: totalSnapshot,
                deliveryAddress: shippingDestination, docId: '',
              ),
            ),
          );
        }
  ...
  void _showSnackBar(String msg, {required bool isError}) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(msg, style: GoogleFonts.poppins(color: const Color(0xff5E1D04), fontSize: 13)),
        backgroundColor: isError ? const Color(0xffF6B55E) : Colors.green.shade400,
